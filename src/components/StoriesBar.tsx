...
const StoriesBar = ({ followedCreators }: StoriesBarProps) => {
  return (
    <div className="w-full pb-8 border-b border-zinc-900/50 mb-8">
          ...
          <div key={creator.id} className="flex flex-col items-center gap-2 shrink-0 cursor-pointer group">
            <div className="w-16 h-16 rounded-full p-[2px] bg-gradient-to-tr from-purple-600 to-pink-500 transition-transform group-hover:scale-105">
              <div className="w-full h-full rounded-full bg-black p-[2px]">
                <div className="w-full h-full rounded-full overflow-hidden relative border border-zinc-900">
                  <Image 
            ...
            <span className="text-[10px] font-black uppercase tracking-tighter text-zinc-400 group-hover:text-white transition-colors">
              {creator.username.length > 8 ? `${creator.username.slice(0, 8)}...` : creator.username}
            </span>
          </div>
        ))}
      </div>
