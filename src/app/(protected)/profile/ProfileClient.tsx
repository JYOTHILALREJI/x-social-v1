...
const ProfileClient = ({ user, platformFee }: ProfileClientProps) => {
  ...
  return (
    <div className="w-full min-h-screen bg-black text-white px-6 md:px-12 pt-10 pb-32">
      ...
      {/* --- TOP PROFILE SECTION --- */}
      <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] items-center lg:items-start gap-8 lg:gap-12 mb-12 border-b border-zinc-900 pb-12">
        {/* Profile Image */}
        <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-zinc-900 border-2 border-zinc-800 relative overflow-hidden shrink-0 mx-auto lg:mx-0">
           <Image 
        ...
        <div className="flex-1 space-y-6 text-center lg:text-left w-full">
          <div className="flex items-center justify-between lg:justify-start gap-4">
            <h1 className="text-4xl font-black italic uppercase tracking-tighter">{user.username}</h1>
            {isVerifiedCreator && (
            ...
            {/* Settings Link for desktop (mobile has it too but keep it simple) */}
            {isVerifiedCreator && (
              <Link href="/settings" className="p-2.5 bg-zinc-900 hover:bg-zinc-800 rounded-xl border border-zinc-800 transition-all hidden lg:block">
          ...
          <div className="flex justify-center lg:justify-start gap-10">
            <div className="flex flex-col"><span className="font-bold text-2xl tracking-tighter">{user._count.posts}</span><span className="text-zinc-500 text-[10px] uppercase font-black tracking-widest">Posts</span></div>
            <div className="flex flex-col"><span className="font-bold text-2xl tracking-tighter">{user._count.reels}</span><span className="text-zinc-500 text-[10px] uppercase font-black tracking-widest">Reels</span></div>
            <div className="flex flex-col"><span className="font-bold text-2xl tracking-tighter">{user.followersCount || 0}</span><span className="text-zinc-500 text-[10px] uppercase font-black tracking-widest">Followers</span></div>
          </div>
        ...
        {!isVerifiedCreator && !isPending && (
          <div className="w-fit min-w-[340px] flex items-center gap-6 bg-zinc-950/80 border border-zinc-800 p-3 pl-8 rounded-full hover:border-purple-600/50 transition-all shadow-2xl shadow-black/40">
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-32">
                 ...
                 <input 
                    ...
                    placeholder="Amount" 
                    className="w-full bg-black border border-zinc-800 rounded-2xl py-3 pl-8 pr-4 text-xs font-black focus:outline-none focus:border-purple-600 focus:ring-4 focus:ring-purple-600/10 transition-all text-white placeholder:text-zinc-800 uppercase" 
                />
              </div>
              <button 
      ...
      {!isVerifiedCreator ? (
        ...
        <div className="space-y-12">
           <CreatorDashboard user={userData as any} platformFee={platformFee} />
                      ...
                      <div key={post.id} className="aspect-square bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800 relative group">
                        ...
                        {post.isPremium && (
                          <div className="absolute top-3 right-3 z-10 flex flex-col items-end gap-1 pointer-events-none">
                            ...
                            <span className="px-2 py-1 bg-black/80 backdrop-blur-md text-purple-400 text-[9px] font-black rounded-lg border border-zinc-900/50">
                      ...
                      <div key={reel.id} className="aspect-[9/16] bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800 relative group">
                        ...
                        {reel.isPremium && (
                          <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-1.5 pointer-events-none">
                            ...
                            <span className="px-3 py-1 bg-black/80 backdrop-blur-md text-purple-400 text-[11px] font-black rounded-xl border border-zinc-900/50">
        ...
        {showConfirm && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-xl bg-black/40">
                <motion.div
                    ...
                    className="bg-zinc-950 border border-zinc-900 rounded-[3rem] p-12 w-full max-w-md shadow-2xl relative overflow-hidden"
                >
                    ...
                    <div className="space-y-6">
                        <div className="p-8 bg-zinc-900 rounded-[2rem] border border-zinc-800 text-center">
                        ...
                        <div className="flex gap-4">
                             <button 
                                ...
                                className="flex-1 py-5 bg-zinc-900 text-zinc-500 font-bold rounded-2xl hover:bg-zinc-800 hover:text-white transition-all shadow-lg active:scale-95 border border-zinc-800"
                             >
                                Cancel
                             </button>
                             <button 
